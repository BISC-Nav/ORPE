...
static void get_tangent_basis(const Eigen::Vector3d& u, Eigen::Vector3d& b1, Eigen::Vector3d& b2) {
    Eigen::Vector3d a = Eigen::Vector3d::UnitX();
    if (std::abs(u(0)) > 0.9) a = Eigen::Vector3d::UnitY();
    b1 = a.cross(u).normalized();
    b2 = u.cross(b1);
}

// 继承自 BaseBinaryEdge
// 维度: 2 (切平面投影是2维的)
// 测量值类型: Eigen::Vector3d (归一化观测射线)
// 顶点0: 3D点
// 顶点1: VertexPose (IMU位姿)
class EdgeMonoRay : public g2o::BaseBinaryEdge<2, Eigen::Vector3d, g2o::VertexSBAPointXYZ, VertexPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    // 构造函数传入相机索引
    EdgeMonoRay(int cam_idx_=0): cam_idx(cam_idx_){
    }
    ...
    void setMeasurement(const Eigen::Vector3d& m) override {
        _measurement = m;
        _measurement.normalize(); // 确保归一化
        get_tangent_basis(_measurement, b1, b2);
    }
    ...
    void computeError() override {
        ...
        // 获取当前估计的相机位姿参数
        // VertexPose 内部维护了 ImuCamPose，它会自动计算 Rcw 和 tcw
        const ImuCamPose& imuCam = VPose->estimate();
        const Eigen::Matrix3d& Rcw = imuCam.Rcw[cam_idx];
        const Eigen::Vector3d& tcw = imuCam.tcw[cam_idx];

        // 将点转到相机坐标系: Pc = Rcw * Pw + tcw
        ...
        // 计算残差: r = B^T * Pc
        // 物理意义: Pc 在观测射线垂直方向上的分量
        _error(0) = b1.dot(Pc) / norm;
        _error(1) = b2.dot(Pc) / norm;
    }
    ...
    virtual void linearizeOplus() override {
        ...
        // 获取必要的旋转和平移
        const Eigen::Matrix3d& Rcw = imuCam.Rcw[cam_idx]; // World -> Camera
        const Eigen::Matrix3d& Rcb = imuCam.Rcb[cam_idx]; // Body -> Camera (外参)
        const Eigen::Matrix3d& Rwb = imuCam.Rwb;          // Body -> World
        const Eigen::Vector3d& twb = imuCam.twb;          // Body Position in World
        
        // 计算中间变量
        Eigen::Vector3d Pw = VPoint->estimate();
        // Pb = Rbw * (Pw - twb)  => Point in Body Frame
        Eigen::Vector3d Pb = Rwb.transpose() * (Pw - twb);
        
        Eigen::Vector3d Pc = Rcw * Pw + imuCam.tcw[cam_idx];

        double norm = Pc.norm();
        if(norm < 1e-6) norm = 1e-6;
        double inv_norm = 1.0 / norm;
        Eigen::Vector3d Pc_norm = Pc * inv_norm;

        // 构造切平面投影矩阵 B^T
        Eigen::Matrix<double, 2, 3> B_T;
        B_T.row(0) = b1;
        B_T.row(1) = b2;

        // 计算当前的归一化残差 (必须在函数内重新计算，不能依赖 _error)
        Eigen::Vector2d current_error;
        current_error(0) = b1.dot(Pc_norm);
        current_error(1) = b2.dot(Pc_norm);

        // --- 修正点：使用精确雅可比 ---
        // J = 1/norm * (B^T - e * Pc_norm^T)
        // 这一项 (e * Pc_norm^T) 保证了沿光线方向的梯度为0
        Eigen::Matrix<double, 2, 3> J_e_Pc = inv_norm * (B_T - current_error * Pc_norm.transpose());

        // -----------------------------------------------------------------
        // Jacobian w.r.t Point (Vertex 0)
        _jacobianOplusXi = J_e_Pc * Rcw;

        // -----------------------------------------------------------------
        // Jacobian w.r.t Pose (Vertex 1)
        Eigen::Matrix<double, 3, 6> dPb_dksi;
        dPb_dksi.block<3,3>(0,0) = Skew(Pb);              // Rotation (ur)
        dPb_dksi.block<3,3>(0,3) = -Eigen::Matrix3d::Identity(); // Translation (ut)

        _jacobianOplusXj = J_e_Pc * Rcb * dPb_dksi;
    }

    // 辅助函数：判断深度是否为正 (用于 Cheirality Check)
