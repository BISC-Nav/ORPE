...
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    ...
    // Mono
    vector<EdgeMonoRay*> vpEdgesMonoRay;
    vpEdgesMonoRay.reserve(nExpectedSize);
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                if(leftIndex != -1 && pKFi->mvuRight[leftIndex]<0)
                {
                    ...
                    cv::Point3f x3D = pKFi->mpCamera->unproject(kpUn.pt);
                    Eigen::Vector3d obs_ray(x3D.x, x3D.y, x3D.z);

                    EdgeMonoRay* e = new EdgeMonoRay(0);

                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
                    e->setMeasurement(obs_ray);

                    // Add here uncerteinty
                    const float unc2 = pKFi->mpCamera->uncertainty2(obs_ray.head<2>());

                    // 1. 获取焦距 (fx)
                    const float fx = pKF->mpCamera->getParameter(0);

                    // 2. 获取当前特征点的像素精度 (1/sigma_pix^2)
                    const float invSigma2_pix = pKF->mvInvLevelSigma2[kpUn.octave];

                    // 3. 计算角度空间的信息矩阵
                    // 角度误差 sigma_angle = sigma_pix / fx
                    // 信息权重 weight = 1 / sigma_angle^2 = (fx^2) / sigma_pix^2 = fx^2 * invSigma2_pix
                    const float information_factor = fx * fx * invSigma2_pix;

                    //std::cout << "information_factor: " << information_factor << std::endl;

                    // 设置信息矩阵 (与深度无关)
                    e->setInformation(Eigen::Matrix2d::Identity() * information_factor);

                    // 4. 设置 Huber 核函数
                    g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
                    e->setRobustKernel(rk);
                    
                    // Delta 应该是无量纲的 Chi2 阈值 (sqrt(5.991) 对应 2自由度 95% 置信度)
                    // thHuberMono 在文件前面已经定义为 sqrt(5.991)
                    rk->setDelta(thHuberMono); 

                    optimizer.addEdge(e);
                    vpEdgesMonoRay.push_back(e);
                    vpEdgeKFMono.push_back(pKFi);
                    vpMapPointEdgeMono.push_back(pMP);
                }
                // Stereo-observation
    ...
    vector<pair<KeyFrame*,MapPoint*> > vToErase;
    vToErase.reserve(vpEdgesMonoRay.size()+vpEdgesStereo.size());

    // Check inlier observations
    // Mono
    for(size_t i=0, iend=vpEdgesMonoRay.size(); i<iend;i++)
    {
        EdgeMonoRay* e = vpEdgesMonoRay[i];
        MapPoint* pMP = vpMapPointEdgeMono[i];
