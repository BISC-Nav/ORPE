...
void UpdaterHelper::get_feature_jacobian_full(std::shared_ptr<State> state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
  ...
  for (auto const &pair : feature.timestamps) {
    ...
    for (size_t m = 0; m < feature.timestamps[pair.first].size(); m++) {
      ...
      // modified: 3D RAY PROJECT ERROR

      //---------------------------------ORPE-----------------------------------

      Eigen::Vector2d uv_meas_norm = feature.uvs_norm[pair.first].at(m).cast<double>();
      Eigen::Vector3d u_vec;
      u_vec << uv_meas_norm(0), uv_meas_norm(1), 1.0;
      u_vec.normalize();

      Eigen::Vector3d b1, b2;
      get_tangent_basis(u_vec, b1, b2);
      ...
      double norm_p = p_FinCi.norm();

      // Residual = Measurement - Estimate
      // Measurement in tangent space is 0 (since B_T is orthogonal to u_vec)
      // Estimate is B_T * (p_FinCi / norm_p)
      // So Residual = 0 - Estimate
      res.block(2 * c, 0, 2, 1) = -B_T * (p_FinCi/norm_p);
      ...
      Eigen::Matrix3d P_proj = Eigen::Matrix3d::Identity() - (p_FinCi * p_FinCi.transpose()) / (norm_p * norm_p);
      Eigen::MatrixXd dz_dpfc = B_T * P_proj / norm_p;
