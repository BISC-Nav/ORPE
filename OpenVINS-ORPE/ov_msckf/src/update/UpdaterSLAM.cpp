...
void UpdaterSLAM::update(std::shared_ptr<State> state, std::vector<std::shared_ptr<Feature>> &feature_vec) {
  ...
  while (it2 != feature_vec.end()) {
    ...
    double depth = feat.p_FinA.norm();

    double res_norm = res.norm();
    double chi2_val = chi2;

    static std::ofstream out_res("/home/nav/output/residual_depth_rpe.txt", std::ios::app);
    if (out_res.is_open()) {
        out_res << std::fixed << std::setprecision(6) 
                << state->_timestamp << " " 
                << feat.featid << " " 
                << depth << " " 
                << res_norm << " " 
  ...
  // ========================= [新增代码开始] =========================
  // 计算雅可比矩阵 Hx_big 的条件数
  // 注意：SVD 计算量较大，如果在实时性要求极高的场景慎用，或者仅在调试模式开启
  if (Hx_big.rows() > 0 && Hx_big.cols() > 0) {
      // 使用 JacobiSVD 计算奇异值 (不需要计算 U 和 V 矩阵，只算奇异值)
      Eigen::JacobiSVD<Eigen::MatrixXd> svd(Hx_big);
      
      // 获取奇异值向量 (Eigen 默认从大到小排列)
      const auto& singularValues = svd.singularValues();
      double max_sv = singularValues(0);
      double min_sv = singularValues(singularValues.size() - 1);
      
      // 计算条件数 (防止除以零)
      double cond_number = (min_sv > 1e-9) ? (max_sv / min_sv) : -1.0;

      // 保存到文件用于后续绘图分析
      // 格式: timestamp condition_number max_singular_value min_singular_value rows cols
      static std::ofstream out_cond("/home/nav/output/jacobian_cond_rpe.txt", std::ios::app);
      if (out_cond.is_open()) {
          out_cond << std::fixed << std::setprecision(6) 
                   << state->_timestamp << " " 
                   << cond_number << " " 
                   << max_sv << " " 
                   << min_sv << " "
                   << Hx_big.rows() << " " 
                   << Hx_big.cols() << std::endl;
      }
      
      // 可选：在终端打印过大的条件数警告
      if (cond_number > 1e6 || cond_number < 0) {
           // PRINT_WARNING(YELLOW "[SLAM-UP]: High Condition Number detected: %.2e\n" RESET, cond_number);
      }
  }
  // ========================= [新增代码结束] =========================
